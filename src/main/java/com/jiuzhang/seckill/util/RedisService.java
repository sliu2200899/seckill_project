...
public class RedisService {
    ...
    public boolean isInLimitMember(long activityId, long userId) {
        Jedis jedisClient = jedisPool.getResource();
        boolean sismember = jedisClient.sismember("seckillActivity_user:" + activityId, String.valueOf(userId));
        jedisClient.close();
        log.info("userId:{} activityId:{} 在限购名单中:{}", userId, activityId, sismember);
        return sismember;
    }
    ...
    public void addLimitMember(long activityId, long userId) {
        Jedis jedisClient = jedisPool.getResource();
        jedisClient.sadd("seckillActivity_user:" + activityId, String.valueOf(userId));
        jedisClient.close();
    }
    ...
    public void removeLimitMember(Long activityId, Long userId) {
        Jedis jedisClient = jedisPool.getResource();
        jedisClient.srem("seckillActivity_user:" + activityId, String.valueOf(userId));
        jedisClient.close();
    }
}
